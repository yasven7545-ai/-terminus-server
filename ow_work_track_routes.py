"""
OW WORK TRACK — DAILY WORKS / COST IMPLICATION WORKFLOW
ONEWEST Property Management System
Blueprint prefix: /ow_work_track
"""
...
# BLUEPRINT
# ─────────────────────────────────────────────────────────
ow_work_track_bp = Blueprint("ow_work_track", __name__)

# ─────────────────────────────────────────────────────────
# PATHS
# ─────────────────────────────────────────────────────────
BASE_DIR                = Path(__file__).parent.resolve()
OW_WORK_TRACK_DATA_FILE = BASE_DIR / "static" / "data" / "ow_work_track_works.json"
OW_WORK_TRACK_LOG_FILE  = BASE_DIR / "static" / "data" / "ow_work_track_mail_log.json"
OW_WORK_TRACK_TMPL_FILE = BASE_DIR / "static" / "data" / "ow_work_track_templates.json"

for _p in [OW_WORK_TRACK_DATA_FILE, OW_WORK_TRACK_LOG_FILE, OW_WORK_TRACK_TMPL_FILE]:
...
DEFAULT_SUBJECTS = {
    "Issue Identification":         "[ONEWEST] Cost Implication Work - Issue Identified: {title}",
    "GM Approval":                  "[ONEWEST] GM Approval Required: {title}",
    "Procurement / Finance Action": "[ONEWEST] Procurement/Finance Action Required: {title}",
    "Management Approval":          "[ONEWEST] Final Management Approval: {title}",
    "Contract Awarded":             "[ONEWEST] Contract Awarded / PO Issued: {title}",
    "Execution & Daily Monitoring": "[ONEWEST] Daily Progress Update - {date}: {title}",
    "Completion & Closure":         "[ONEWEST] Work Completed and Closed: {title}",
}
...
def _owt_prop(f):
    ...
    def w(*a, **kw):
        try:
            if session.get("active_property") != "ONEWEST" and session.get("role") != "admin":
                return jsonify({"success": False, "error": "ONEWEST access required"}), 403
        except RuntimeError:
            return f(*a, **kw)
        return f(*a, **kw)
    return w

def _owt_current_user():
...
# ─────────────────────────────────────────────────────────
def _owt_load():
    if not OW_WORK_TRACK_DATA_FILE.exists():
        return []
    try:
        with open(OW_WORK_TRACK_DATA_FILE, "r", encoding="utf-8") as fh:
            return json.load(fh)
    except Exception:
        return []

def _owt_save(data):
    OW_WORK_TRACK_DATA_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(OW_WORK_TRACK_DATA_FILE, "w", encoding="utf-8") as fh:
        json.dump(data, fh, indent=2, default=str)

def _owt_load_templates():
    if not OW_WORK_TRACK_TMPL_FILE.exists():
        return {}
    try:
        with open(OW_WORK_TRACK_TMPL_FILE, "r", encoding="utf-8") as fh:
            return json.load(fh)
    except Exception:
        return {}

def _owt_save_templates(data):
    with open(OW_WORK_TRACK_TMPL_FILE, "w", encoding="utf-8") as fh:
        json.dump(data, fh, indent=2)

def _owt_log_mail(work_id, stage, recipients, status, error=""):
    log = []
    if OW_WORK_TRACK_LOG_FILE.exists():
        try:
            with open(OW_WORK_TRACK_LOG_FILE, "r", encoding="utf-8") as fh:
                log = json.load(fh)
        except Exception:
            log = []
    log.append({
    ...
    with open(OW_WORK_TRACK_LOG_FILE, "w", encoding="utf-8") as fh:
...
def _owt_build_ctx(work, extra=None):
    ctx = {
...
def _owt_resolve_recipients(depts):
    seen, result = set(), []
    for d in depts:
        for r in OW_DEPT_EMAILS.get(d, OW_DEPT_EMAILS["All"]):
            if r not in seen:
                seen.add(r)
                result.append(r)
    return result
...
# EMAIL HELPERS
# ─────────────────────────────────────────────────────────
def _owt_build_html(work, subject, plain_body):
    stage = work.get("current_stage", "")
    ...
    return (
        ...
        f"ONEWEST &#8212; Works &amp; Cost Tracker</h2>"
...
def _owt_send_smtp(recipients, subject, plain_body, html_body=None):
    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = subject
        msg["From"]    = f"ONEWEST Works Tracker <{_SMTP_USER}>"
...
@ow_work_track_bp.route("/works", methods=["GET"])
@_owt_auth
@_owt_prop
def ow_work_track_list():
    works      = _owt_load()
...
@ow_work_track_bp.route("/works", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_create():
    ...
    work  = {
        ...
        "identified_by":  data.get("identified_by", _owt_current_user()),
        ...
        "created_at":     now,
        "updated_at":     now,
        "created_by":     _owt_current_user(),
    }
    work["history"].append({
        "stage":     "Issue Identification",
        "action":    "Work Created",
        "by":        _owt_current_user(),
        "timestamp": now,
...
@ow_work_track_bp.route("/works/<work_id>", methods=["GET"])
@_owt_auth
@_owt_prop
def ow_work_track_get(work_id):
    work = next((w for w in _owt_load() if w["id"] == work_id), None)
    if not work:
        return jsonify({"success": False, "error": "Work not found"}), 404
    return jsonify({"success": True, "work": work})


@ow_work_track_bp.route("/works/<work_id>", methods=["PUT"])
@_owt_auth
@_owt_prop
def ow_work_track_update(work_id):
    data  = request.get_json(silent=True) or {}
    works = _owt_load()
...
@ow_work_track_bp.route("/works/<work_id>", methods=["DELETE"])
@_owt_auth
@_owt_prop
def ow_work_track_delete(work_id):
    works  = _owt_load()
    before = len(works)
...
@ow_work_track_bp.route("/works/<work_id>/stage", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_stage(work_id):
    data  = request.get_json(silent=True) or {}
    works = _owt_load()
    idx   = next((i for i, w in enumerate(works) if w["id"] == work_id), None)
    if idx is None:
...
@ow_work_track_bp.route("/works/<work_id>/daily-update", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_daily_update(work_id):
    data  = request.get_json(silent=True) or {}
    works = _owt_load()
    idx   = next((i for i, w in enumerate(works) if w["id"] == work_id), None)
    if idx is None:
...
@ow_work_track_bp.route("/trigger-mail", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_trigger_mail():
    data    = request.get_json(silent=True) or {}
    ...
    recipients = _owt_resolve_recipients(depts)
    ctx        = _owt_build_ctx(work)
    subject    = (subj_ov or DEFAULT_SUBJECTS.get(stage, "[ONEWEST] Work Update: {title}")).format(**ctx)
    body       = (body_ov or DEFAULT_BODIES.get(stage, "Work update for {title}.")).format(**ctx)
    html       = _owt_build_html(work, subject, body)
    result     = _owt_send_smtp(recipients, subject, body, html)
    _owt_log_mail(work_id, stage, recipients,
...
@ow_work_track_bp.route("/emergency-mail", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_emergency_mail():
    data    = request.get_json(silent=True) or {}
    work_id = data.get("work_id", "")
    works   = _owt_load()
    work    = next((w for w in works if w["id"] == work_id), None)
    if not work:
        return jsonify({"success": False, "error": "Work not found"}), 404
    subject = data.get("subject") or f"[ONEWEST][EMERGENCY] Immediate Attention: {work.get('title','')}"
    body    = data.get("body")
    if not body:
        ctx  = _owt_build_ctx(work)
        body = (
            "EMERGENCY NOTICE\n\n"
            "Urgent action required for the following work at ONEWEST.\n\n"
    ...
    html   = _owt_build_html(work, subject, body)
    result = _owt_send_smtp(OW_EMERGENCY_EMAILS, subject, body, html)
    _owt_log_mail(work_id, "EMERGENCY", OW_EMERGENCY_EMAILS,
                  "sent" if result["success"] else "failed",
                  result.get("error", ""))
    if result["success"]:
        return jsonify({"success": True, "recipients": OW_EMERGENCY_EMAILS,
                        "count": len(OW_EMERGENCY_EMAILS)})
    return jsonify({"success": False, "error": result["error"]}), 500


@ow_work_track_bp.route("/preview-mail", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_preview_mail():
    data    = request.get_json(silent=True) or {}
...
@ow_work_track_bp.route("/mail-log", methods=["GET"])
@_owt_auth
@_owt_prop
def ow_work_track_mail_log():
    if not OW_WORK_TRACK_LOG_FILE.exists():
...
@ow_work_track_bp.route("/default-templates", methods=["GET"])
@_owt_auth
@_owt_prop
def ow_work_track_default_templates():
    return jsonify({
        "success":     True,
        ...
        "departments": list(OW_DEPT_EMAILS.keys()),
    })


@ow_work_track_bp.route("/templates", methods=["GET"])
@_owt_auth
@_owt_prop
def ow_work_track_get_templates():
    return jsonify({"success": True, "templates": _owt_load_templates()})


@ow_work_track_bp.route("/templates", methods=["POST"])
@_owt_auth
@_owt_prop
def ow_work_track_save_template():
    data  = request.get_json(silent=True) or {}
    stage = data.get("stage", "")
    if not stage:
        return jsonify({"success": False, "error": "Stage required"}), 400
    templates        = _owt_load_templates()
    templates[stage] = {
...
@ow_work_track_bp.route("/stats", methods=["GET"])
@_owt_auth
@_owt_prop
def ow_work_track_stats():
    works = _owt_load()
    return jsonify({
        "success":     True,
        "total":       len(works),
...
@ow_work_track_bp.route("/health", methods=["GET"])
def ow_work_track_health():
    return jsonify({
        "success":   True,
        "module":    "ow_work_track",
        "property":  "ONEWEST",
        "timestamp": datetime.now().isoformat(),
    })
...
# Set inside ow_work_track_register(app) — NEVER at module level.
_owt_flask_app      = None
_owt_scheduler_inst = None


def ow_work_track_daily_auto_mail():
    """
    ...
    print(f"\n{'='*55}")
    print(f"[OW AUTO-MAIL] Triggered at {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')} IST")
    print(f"{'='*55}")
    ...
    def _run():
        try:
            ...
            print(f"[OW AUTO-MAIL] Active — Works: {len(active_works)} | Obs: {len(active_obs)}")

            if not active_works and not active_obs:
                print("[OW AUTO-MAIL] Nothing active — skipping digest")
                return

            # ── Build ONE digest email ─────────────────────────
            now_str   = datetime.now().strftime("%d %b %Y, %I:%M %p IST")
            date_str  = datetime.now().strftime("%d %b %Y")
            recipients = _owt_resolve_recipients(["All"])

            subject = f"[ONEWEST] Daily Status Digest — {len(active_works)} Works · {len(active_obs)} Observations — {date_str}"

            # ── Plain text digest ──────────────────────────────
            lines = [
                f"ONEWEST — Daily Status Digest",
                f"Generated : {now_str}",
                f"{'═'*52}",
                "",
            ]
...
<div style="max-width:700px;margin:0 auto;padding:24px 16px;">
  ...
  <div style="background:#0c1018;border:1px solid #1e2636;border-radius:10px;padding:20px 24px;margin-bottom:20px;border-top:3px solid #f5a623;">
    <div style="font-size:20px;font-weight:800;color:#f5a623;letter-spacing:.02em;">ONEWEST</div>
  ...
  <div style="text-align:center;padding:20px 16px;border-top:1px solid #1e2636;margin-top:8px;">
    ...
    <p style="font-size:10px;color:#8899aa;margin:10px 0 0;">
      ONEWEST &nbsp;·&nbsp; emerZhent Property Management Services
    </p>
  </div>

</div></body></html>"""

            result = _owt_send_smtp(recipients, subject, plain_body, html_body)
            status = "digest-sent" if result["success"] else "digest-failed"
            all_ids = [w["id"] for w in active_works] + [o["id"] for o in active_obs]
            _owt_log_mail("DIGEST", "daily-digest", recipients, status, result.get("error",""))
            print(f"[OW AUTO-MAIL] {'✅ DIGEST SENT' if result['success'] else '❌ DIGEST FAILED'} → {len(recipients)} recipients | {len(active_works)} works + {len(active_obs)} obs")

        except Exception as exc:
            print(f"[OW AUTO-MAIL] ❌ EXCEPTION: {exc}")
            _tb.print_exc()
            _owt_log_mail("AUTO", "all-stages", [], "auto-error", str(exc))

    if _owt_flask_app is not None:
        with _owt_flask_app.app_context():
            _run()
    else:
        print("[OW AUTO-MAIL] ⚠️  _owt_flask_app is None — "
              "ow_work_track_register(app) was not called!")
        _run()
...
def ow_work_track_register(app):
    """
    ...
    """
    global _owt_flask_app, _owt_scheduler_inst

    # 1. Store app reference FIRST
    _owt_flask_app = app

    # 2. Register blueprint
    app.register_blueprint(ow_work_track_bp, url_prefix="/ow_work_track")
    print("✅ OW Work Track Blueprint registered at /ow_work_track")

    # 3. Start scheduler only once
    if _owt_scheduler_inst is not None:
        print("ℹ️  OW Work Track Scheduler already running — skipped.")
        return
    ...
    try:
        ...
        _owt_scheduler_inst = BackgroundScheduler(
        ...
        _owt_scheduler_inst.add_job(
            func=ow_work_track_daily_auto_mail,
            trigger=CronTrigger(hour=8, minute=0, timezone="Asia/Kolkata"),
            id="ow_work_track_8 AM",
            name="ONEWEST Works Daily Mail 08:00 IST",
            replace_existing=True,
        )
        _owt_scheduler_inst.start()

        # ✅ Fixed: ID matches the one used in add_job above ("ow_work_track_8 AM")
        job = _owt_scheduler_inst.get_job("ow_work_track_8 AM")  
        next_run = (job.next_run_time.strftime("%Y-%m-%d %H:%M:%S %Z")
                    if job and job.next_run_time else "unknown")
        print(f"✅ OW Work Track Scheduler started — next run: {next_run}")

    except Exception as exc:
        import traceback
        print(f"⚠️  OW Work Track Scheduler setup failed: {exc}")
        traceback.print_exc()
...
OW_OBSERVATIONS_FILE = BASE_DIR / "static" / "data" / "ow_observations.json"
OW_OBS_UPLOADS_DIR   = BASE_DIR / "uploads" / "ow_observations"
OW_OBS_UPLOADS_DIR.mkdir(parents=True, exist_ok=True)
OW_OBSERVATIONS_FILE.parent.mkdir(parents=True, exist_ok=True)
...
OBS_SUBJECTS = {
    "Observation Raised by GM/Management": "[ONEWEST][OBS] New Observation Identified: {title}",
    "PM Review & Response": "[ONEWEST][OBS] PM Response Submitted: {title}",
    "GM Verification": "[ONEWEST][OBS] GM Verification Required: {title}",
    "Management Approval": "[ONEWEST][OBS] Management Approval Required: {title}",
    "Procurement/Finance Action": "[ONEWEST][OBS] Procurement/Finance Action Completed: {title}",
    "Work In Progress": "[ONEWEST][OBS] Work In Progress Update: {title}",
    "Completed": "[ONEWEST][OBS] Observation Closed: {title}",
}
...
def _obs_load():
    if not OW_OBSERVATIONS_FILE.exists():
        return []
    try:
        with open(OW_OBSERVATIONS_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return []
...
def _obs_save(data):
    OW_OBSERVATIONS_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(OW_OBSERVATIONS_FILE, "w", encoding="utf-8") as f:
...
def _obs_build_html(ob, subject, plain_body):
    ...
    return (
        ...
        f'text-transform:uppercase;font-family:Arial,sans-serif;">ONEWEST &#8212; OBSERVATIONS</p>'
...
def _obs_send_mail(ob, stage, override_body=None):
    ctx        = _obs_ctx(ob)
    subject    = OBS_SUBJECTS.get(stage, "[ONEWEST][OBS] Update: {title}").format(**ctx)
    body       = override_body or OBS_BODIES.get(stage, "Observation update.").format(**ctx)
    recipients = _owt_resolve_recipients(OBS_STAGE_DEPTS.get(stage, ["All"]))
    html       = _obs_build_html(ob, subject, body)
    result     = _owt_send_smtp(recipients, subject, body, html)
    _owt_log_mail(ob["id"], f"OBS|{stage}", recipients,
                  "sent" if result["success"] else "failed",
                  result.get("error",""))
...
@ow_work_track_bp.route("/observations", methods=["GET"])
@_owt_auth
@_owt_prop
def obs_list():
...
@ow_work_track_bp.route("/observations", methods=["POST"])
@_owt_auth
@_owt_prop
def obs_create():
    ...
    if image and image.filename:
        ...
        if ext in {"png","jpg","jpeg","gif","webp","pdf"}:
            ...
            image.save(str(OW_OBS_UPLOADS_DIR / fname))
            image_path = f"/ow_work_track/obs-image/{fname}"
    ...
    ob = {
        ...
        "raised_by":         data.get("raised_by", _owt_current_user()),
...
@ow_work_track_bp.route("/observations/<obs_id>", methods=["GET"])
@_owt_auth
@_owt_prop
def obs_get(obs_id):
...
@ow_work_track_bp.route("/observations/<obs_id>", methods=["PUT"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>", methods=["DELETE"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/pm-response", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/gm-verify", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/management-approve", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/approve", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/procurement-complete", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/work-progress", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/complete", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/stage", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/trigger-mail", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/observations/<obs_id>/emergency-mail", methods=["POST"])
@_owt_auth
@_owt_prop
def obs_emergency_mail(obs_id):
    ...
    ctx     = _obs_ctx(ob)
    subject = data.get("subject") or f"[ONEWEST][EMERGENCY][OBS] Immediate Attention: {ob.get('title','')}"
    body    = data.get("body") or (
        "EMERGENCY NOTICE\n\n"
        "Urgent attention required for the following observation at ONEWEST.\n\n"
    ...
    html       = _obs_build_html(ob, subject, body)
    result     = _owt_send_smtp(recipients, subject, body, html)
    _owt_log_mail(obs_id, "OBS|EMERGENCY", recipients,
                  "sent" if result["success"] else "failed",
                  result.get("error", ""))
    if result["success"]:
        return jsonify({"success": True, "recipients": recipients, "count": len(recipients)})
...
@ow_work_track_bp.route("/observations/<obs_id>/preview-mail", methods=["POST"])
@_owt_auth
@_owt_prop
...
@ow_work_track_bp.route("/obs-image/<filename>")
@_owt_auth
@_owt_prop
def obs_image(filename):
    from flask import send_from_directory
    return send_from_directory(str(OW_OBS_UPLOADS_DIR), secure_filename(filename))


# ── Stats ──────────────────────────────────────────────────

@ow_work_track_bp.route("/observations/stats", methods=["GET"])
@_owt_auth
@_owt_prop
def obs_stats():
    obs = _obs_load()
