...
def require_property(property_name):
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if session.get("active_property") != property_name and session.get("role") != "admin":
                abort(403)
...
if not OW_TECHNICIANS_JSON.exists():
    with open(OW_TECHNICIANS_JSON, 'w', encoding='utf-8') as f:
        json.dump({
            "technicians": [
                {"id": "T001", "name": "Venu", "phone": "+917981397300", "specialization": "Electrical"},
...
OW_EMAIL_RECEIVERS = [
    ...
    "engineering.services@emerzhent.com"
]
...
@app.route("/ow_mail_reminder")
@login_required
def ow_mail_reminder_page():
    session['active_property'] = 'ONEWEST'
    return render_template("ow_mail_reminder.html")


"""
OW MAIL REMINDER SERVER — ow_mail_rem_server.py
ONEWEST Property Management Email Template Backend
Prefix: ow_mail | Property: ONEWEST

HOW TO REGISTER IN server.py
─────────────────────────────
1. Near the top imports section add:
       from ow_mail_rem_server import ow_mail_bp

2. After safe_register calls add:
       safe_register("ow_mail_rem_server", "ow_mail_bp", url_prefix="/ow_mail")

3. Add this route near your other ONEWEST routes:
       @app.route("/ow_mail_reminder")
       @login_required
       def ow_mail_reminder_page():
           session['active_property'] = 'ONEWEST'
...
4. Copy ow_mail_reminder.html  →  my_dashboard/templates/ow_mail_reminder.html
"""
...
# BLUEPRINT
# ─────────────────────────────────────────────────────────
ow_mail_bp = Blueprint("ow_mail_bp", __name__)

# ─────────────────────────────────────────────────────────
# PATHS & STORAGE
# ─────────────────────────────────────────────────────────
BASE_DIR         = Path(__file__).parent.resolve()
OW_MAIL_DATA_DIR = BASE_DIR / "static" / "data" / "OW" / "mail_reminders"
OW_MAIL_DATA_DIR.mkdir(parents=True, exist_ok=True)

OW_MAIL_RECORDS_FILE = OW_MAIL_DATA_DIR / "ow_mail_records.json"
OW_MAIL_LOG_FILE     = OW_MAIL_DATA_DIR / "ow_mail_send_log.json"

# ─────────────────────────────────────────────────────────
# EMAIL CONFIG
# ─────────────────────────────────────────────────────────
OW_SMTP_SERVER  = "smtp.gmail.com"
OW_SMTP_PORT    = 587
OW_SENDER_EMAIL = "maintenance.slnterminus@gmail.com"
OW_SENDER_PASS  = "xaottgrqtqnkouqn"

OW_DEFAULT_RECEIVERS = [
    "maintenance.slnterminus@gmail.com",
    "yasven7545@gmail.com",
    "engineering@terminus-global.com",
    "kiran@terminus-global.com",
...
OW_ROLE_EMAILS = {
    "General Manager":         ["gm@terminus-global.com"],
    "Procurement Manager":     ["procurement@terminus-global.com"],
    "Facilities Manager":      ["facilities@terminus-global.com"],
    "Operations Manager":      ["operations@terminus-global.com"],
    "Property Manager":        ["pm@terminus-global.com"],
    "Engineering Head":        ["engineering@terminus-global.com"],
    "Housekeeping Supervisor": ["housekeeping@terminus-global.com"],
    "Security Head":           ["security@terminus-global.com"],
    "Finance Controller":      ["finance@terminus-global.com"],
    "All Departments":         OW_DEFAULT_RECEIVERS,
}

# ─────────────────────────────────────────────────────────
# STORAGE HELPERS
# ─────────────────────────────────────────────────────────
def ow_mail_load_records():
    if not OW_MAIL_RECORDS_FILE.exists():
        return []
    try:
        with open(OW_MAIL_RECORDS_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            return data if isinstance(data, list) else []
    except Exception as e:
        print(f"ow_mail_load_records error: {e}")
        return []


def ow_mail_save_records(records):
    try:
        OW_MAIL_DATA_DIR.mkdir(parents=True, exist_ok=True)
        with open(OW_MAIL_RECORDS_FILE, "w", encoding="utf-8") as f:
            json.dump(records, f, indent=2, ensure_ascii=False)
        return True
    except Exception as e:
        print(f"ow_mail_save_records error: {e}")
        return False


def ow_mail_append_log(record_id, recipients, variant, status, error=""):
    try:
        log = []
        if OW_MAIL_LOG_FILE.exists():
            try:
                with open(OW_MAIL_LOG_FILE, "r", encoding="utf-8") as f:
                    raw = json.load(f)
        ...
        OW_MAIL_DATA_DIR.mkdir(parents=True, exist_ok=True)
        with open(OW_MAIL_LOG_FILE, "w", encoding="utf-8") as f:
            json.dump(log[-500:], f, indent=2)
    except Exception as e:
        print(f"ow_mail_append_log error: {e}")


# ─────────────────────────────────────────────────────────
# EMAIL BODY BUILDERS
# ─────────────────────────────────────────────────────────
def _ow_task_block(tasks):
    if not tasks:
        return "  [No tasks listed]"
    icons = {"Pending": "⏳", "In Progress": "🔄", "Completed": "✅", "Overdue": "🔴"}
    lines = []
    for i, t in enumerate(tasks, 1):
        icon = icons.get(t.get("status", "Pending"), "⏳")
        line = f"  {i}. {icon} {t.get('name', '')}"
        if t.get("assigned"):
            line += f" [{t['assigned']}]"
        if t.get("due"):
            line += f" · Due: {t['due']}"
        line += f" — {t.get('status', 'Pending')}"
        lines.append(line)
    return "\n".join(lines)


def _ow_fmt_date(date_str):
    try:
...
def ow_mail_build_body(rec):
    variant    = rec.get("variant", "daily")
    d          = _ow_fmt_date(rec.get("date", ""))
    prop       = rec.get("property", "ONEWEST")
    role       = rec.get("role", "[Recipient Role]")
    subj       = rec.get("subject", "")
    email_addr = rec.get("email", "")
    completion = rec.get("completion", "[COMPLETION %]")
    deadline   = rec.get("deadline", "[DEADLINE]")
    body_note  = rec.get("body", "")
    remarks    = rec.get("remarks", "[ To be completed by department head ]")
    sender     = rec.get("sender", "[Sender Name]")
    s_title    = rec.get("senderTitle", "[Title / Department]")
    tasks      = rec.get("tasks", [])
    tb         = _ow_task_block(tasks)
    sep        = "-" * 54
    sep2       = "-" * 37
    to_line    = role + (" — " + email_addr if email_addr else "")
    ...
    try:
        if variant == "daily":
            return (
                f"SUBJECT: {subj}\n\n"
                f"TO: {to_line}\nDATE: {d}\nPROPERTY: {prop}\n\n"
                f"{sep}\nDAILY PROPERTY ACTIVITIES - REMINDER NOTICE\n{sep}\n\n"
                f"Dear {role},\n\n"
                f"This is your daily operational reminder for {prop} dated {d}. "
                f"Please review the assigned facilities tasks below and ensure all items "
                f"are actioned within the stipulated timeframes.\n\n"
                f"DAILY TASKS & FACILITIES ACTIVITIES:\n{sep2}\n{tb}\n\n"
                f"OVERALL COMPLETION: {completion}\n\n"
                f"ACTION REQUIRED: Acknowledge by {deadline} and confirm status updates."
                f"{note_block}\n\n"
                f"{sep}\nREMARKS (Mandatory - Department Head)\n{sep}\n{remarks}\n\n"
                f"Warm regards,\n{sender}\n{s_title}\n{prop} - Property Management Division"
            )

        if variant == "completion":
            done = str(completion).strip() == "100%"
            note = ("All tasks completed successfully." if done
                    else f"Outstanding tasks must be resolved by {deadline}.")
            return (
                f"SUBJECT: {subj}\n\n"
                f"TO: {to_line}\nDATE: {d}\nPROPERTY: {prop}\n\n"
                f"{sep}\nTASK COMPLETION UPDATE - STATUS REPORT\n{sep}\n\n"
                f"Dear {role},\n\n"
                f"Completion status report for {prop} as of {d}.\n\n"
                f"COMPLETION SUMMARY:\n{sep2}\n{tb}\n\n"
                f"OVERALL COMPLETION: {completion}\n\n{note}"
                f"{note_block}\n\n"
                f"{sep}\nREMARKS (Mandatory - Department Head)\n{sep}\n{remarks}\n\n"
                f"NEXT CHECK-IN: {deadline}\n\n"
                f"Regards,\n{sender}\n{s_title}\n{prop} - Property Management Division"
            )

        if variant == "escalation":
            overdue = rec.get("overdue", "[DAYS OVERDUE]")
            esc_to  = rec.get("escalateTo", "[ESCALATION AUTHORITY]")
            cons    = rec.get("consequence", "[Action if unresolved]")
            return (
                f"SUBJECT: {subj}\n\n"
                f"TO: {to_line}\nCC: {esc_to}\nDATE: {d}\nPROPERTY: {prop}\n\n"
                f"*** {sep} ***\nESCALATION NOTICE - OVERDUE TASKS\n*** {sep} ***\n\n"
                f"Dear {role},\n\n"
                f"This notice escalates overdue tasks at {prop}. "
                f"As of {d}, items are {overdue} overdue. Copied to {esc_to}.\n\n"
                f"OVERDUE TASKS:\n{sep2}\n{tb}\n\n"
                f"OVERALL COMPLETION: {completion} (Target: 100%)\n"
                f"DAYS OVERDUE: {overdue}\n\n"
                f"All tasks must be completed by {deadline}.\nIf unresolved: {cons}"
                f"{note_block}\n\n"
                f"{sep}\nREMARKS (Mandatory - Immediate Response Required)\n{sep}\n{remarks}\n\n"
                f"URGENT - Please acknowledge immediately with an action plan.\n\n"
                f"Regards,\n{sender}\n{s_title}\n{prop} - Property Management Division"
            )

    except Exception as e:
        print(f"ow_mail_build_body error: {e}")
        traceback.print_exc()

    return "[Email body generation failed]"


def ow_mail_build_html(rec, plain_body):
    try:
        variant = rec.get("variant", "daily")
        colors  = {"daily": "#4d9966", "completion": "#4d9966", "escalation": "#b85c5c"}
        labels  = {"daily": "DAILY REMINDER", "completion": "COMPLETION UPDATE", "escalation": "ESCALATION NOTICE"}
        accent  = colors.get(variant, "#4d9966")
        label   = labels.get(variant, "REMINDER")
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#000000;border:1px solid #2a352a;border-radius:8px;overflow:hidden;max-width:100%;">
<tr><td style="background:#000000;border-bottom:3px solid {accent};padding:16px 22px;">
<table width="100%"><tr>
<td><div style="font-size:16px;font-weight:800;color:#c8d8c8;">PropertyOps</div>
<div style="font-size:10px;color:#FFFFFF;letter-spacing:.1em;text-transform:uppercase;font-family:'Courier New',monospace;">
ONEWEST · Email Reminder System</div></td>
<td align="right"><span style="background:{accent}22;border:1px solid {accent}55;color:{accent};
font-size:10px;padding:3px 10px;border-radius:12px;font-family:'Courier New',monospace;">{label}</span></td>
</tr></table></td></tr>
<tr><td style="padding:22px;color:#c8d8c8;font-size:13px;line-height:1.85;">{body_html}</td></tr>
<tr><td style="background:#000000;border-top:1px solid #2a352a;padding:10px 22px;">
<span style="font-size:10px;color:#4a6450;font-family:'Courier New',monospace;">
Property Daily Reminders · ONEWEST · {ts}</span></td></tr>
</table></td></tr></table>
</body></html>"""
    except Exception as e:
        print(f"ow_mail_build_html error: {e}")
        return f"<pre>{plain_body}</pre>"


# ─────────────────────────────────────────────────────────
# SMTP SEND
# ─────────────────────────────────────────────────────────
def ow_mail_smtp_send(to_emails, subject, plain_body, html_body):
    try:
        msg            = MIMEMultipart("alternative")
        msg["Subject"] = subject
        msg["From"]    = formataddr(("ONEWEST PropertyOps", OW_SENDER_EMAIL))
        msg["To"]      = ", ".join(to_emails)
        msg.attach(MIMEText(plain_body, "plain", "utf-8"))
        msg.attach(MIMEText(html_body,  "html",  "utf-8"))
        with smtplib.SMTP(OW_SMTP_SERVER, OW_SMTP_PORT, timeout=20) as srv:
            srv.ehlo()
            srv.starttls()
            srv.login(OW_SENDER_EMAIL, OW_SENDER_PASS)
            srv.sendmail(OW_SENDER_EMAIL, to_emails, msg.as_string())
        print(f"ow_mail: sent to {to_emails}")
        return {"success": True, "error": ""}
    except Exception as e:
        print(f"ow_mail SMTP error: {e}")
        return {"success": False, "error": str(e)}


# ─────────────────────────────────────────────────────────
# API ROUTES
# All routes have full try/except — nothing will cause an
# unhandled 500 that cascades to error.html
# ─────────────────────────────────────────────────────────

@ow_mail_bp.route("/records", methods=["GET"])
def ow_mail_list_records():
    try:
        records = ow_mail_load_records()
        return jsonify({"success": True, "records": records, "total": len(records)})
    except Exception as e:
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e), "records": [], "total": 0}), 500


@ow_mail_bp.route("/records/<path:record_id>", methods=["GET"])
def ow_mail_get_record(record_id):
    try:
        records = ow_mail_load_records()
...
@ow_mail_bp.route("/records", methods=["POST"])
def ow_mail_create_record():
    try:
        data = request.get_json(silent=True) or {}
        if not data.get("property") or not data.get("role") or not data.get("sender"):
            return jsonify({"success": False, "error": "property, role and sender are required"}), 400
        records = ow_mail_load_records()
        new_id  = f"OWM_{datetime.now().strftime('%Y%m%d%H%M%S%f')[:18]}"
        record  = {
            "id":          new_id,
            "variant":     data.get("variant", "daily"),
            "date":        data.get("date", datetime.now().strftime("%Y-%m-%d")),
            "property":    data.get("property", "ONEWEST"),
            "role":        data.get("role", ""),
            ...
            "escalateTo":  data.get("escalateTo", ""),
            "consequence": data.get("consequence", ""),
            "createdAt":   datetime.now().isoformat(),
            "updatedAt":   datetime.now().isoformat(),
            "createdBy":   session.get("user", "system"),
        }
        records.append(record)
        ow_mail_save_records(records)
        print(f"ow_mail: created {new_id}")
        return jsonify({"success": True, "record": record, "id": new_id}), 201
    except Exception as e:
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500


@ow_mail_bp.route("/records/<path:record_id>", methods=["PUT"])
def ow_mail_update_record(record_id):
    try:
        data    = request.get_json(silent=True) or {}
        records = ow_mail_load_records()
        ...
        if idx is None:
            return jsonify({"success": False, "error": "Record not found"}), 404
        for key in ["variant","date","property","role","email","subject",
                    "tasks","completion","deadline","body","remarks",
                    "sender","senderTitle","overdue","escalateTo","consequence"]:
            if key in data:
                records[idx][key] = data[key]
        records[idx]["updatedAt"] = datetime.now().isoformat()
        records[idx]["updatedBy"] = session.get("user", "system")
        ow_mail_save_records(records)
        print(f"ow_mail: updated {record_id}")
        return jsonify({"success": True, "record": records[idx]})
    except Exception as e:
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500


@ow_mail_bp.route("/records/<path:record_id>", methods=["DELETE"])
def ow_mail_delete_record(record_id):
    try:
        records      = ow_mail_load_records()
        ...
        if len(records) == original_len:
            return jsonify({"success": False, "error": "Record not found"}), 404
        ow_mail_save_records(records)
        print(f"ow_mail: deleted {record_id}")
...
@ow_mail_bp.route("/preview", methods=["POST"])
def ow_mail_preview():
    try:
        rec   = request.get_json(silent=True) or {}
        plain = ow_mail_build_body(rec)
        html  = ow_mail_build_html(rec, plain)
        return jsonify({"success": True, "plain": plain, "html": html})
    except Exception as e:
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500


@ow_mail_bp.route("/send/<path:record_id>", methods=["POST"])
def ow_mail_send(record_id):
    try:
        records = ow_mail_load_records()
        record  = next((r for r in records if r.get("id") == record_id), None)
        if not record:
            return jsonify({"success": False, "error": f"Record not found: {record_id}"}), 404

        body_data   = request.get_json(silent=True) or {}
        override    = body_data.get("recipients", [])
        role_emails = OW_ROLE_EMAILS.get(record.get("role", ""), [])
        manual      = [record["email"]] if record.get("email") else []
        recipients  = override or list(set(role_emails + manual)) or OW_DEFAULT_RECEIVERS

        subject    = record.get("subject") or f"[{record.get('variant','').upper()}] ONEWEST Property Reminder"
        plain_body = ow_mail_build_body(record)
        html_body  = ow_mail_build_html(record, plain_body)
        result     = ow_mail_smtp_send(recipients, subject, plain_body, html_body)

        ow_mail_append_log(record_id, recipients, record.get("variant",""),
        ...
        if result["success"]:
            ...
            if idx is not None:
                records[idx].update({
                    "lastSentAt": datetime.now().isoformat(),
                    "lastSentTo": recipients,
                    "lastSentBy": session.get("user", "system"),
                })
                ow_mail_save_records(records)
            return jsonify({
                "success":    True,
...
@ow_mail_bp.route("/send-quick", methods=["POST"])
def ow_mail_send_quick():
    try:
        rec = request.get_json(silent=True) or {}
        if not rec.get("role") or not rec.get("sender"):
            return jsonify({"success": False, "error": "role and sender required"}), 400
        role_emails = OW_ROLE_EMAILS.get(rec.get("role",""), [])
        manual      = [rec["email"]] if rec.get("email") else []
        recipients  = list(set(role_emails + manual)) or OW_DEFAULT_RECEIVERS
        subject     = rec.get("subject") or "ONEWEST Property Reminder"
        plain_body  = ow_mail_build_body(rec)
        html_body   = ow_mail_build_html(rec, plain_body)
        result      = ow_mail_smtp_send(recipients, subject, plain_body, html_body)
        ow_mail_append_log("quick", recipients, rec.get("variant",""),
                           "sent" if result["success"] else "failed",
                           result.get("error",""))
        if result["success"]:
            return jsonify({
...
@ow_mail_bp.route("/send-log", methods=["GET"])
def ow_mail_get_send_log():
    try:
        if not OW_MAIL_LOG_FILE.exists():
        ...
        traceback.print_exc()
        return jsonify({"success": True, "log": [], "warning": str(e)})


@ow_mail_bp.route("/health", methods=["GET"])
def ow_mail_health():
    try:
        records = ow_mail_load_records()
        return jsonify({
            "success":   True,
            "module":    "ow_mail_rem_server",
            "property":  "ONEWEST",
            "records":   len(records),
            "data_dir":  str(OW_MAIL_DATA_DIR),
            "timestamp": datetime.now().isoformat(),
        })
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
# REGISTRATION HELPER
# ─────────────────────────────────────────────────────────
def ow_mail_register(app):
    app.register_blueprint(ow_mail_bp, url_prefix="/ow_mail")
    print("ow_mail_rem_server: registered at /ow_mail")


@app.route("/ow_api/mail/trigger-daily", methods=["POST"])
@login_required
@require_property("ONEWEST")
def ow_mail_trigger_daily_manual():
    """Manually trigger daily mail (for testing)"""
    try:
        result = ow_send_daily_ppm_mail()
        return jsonify(result)
    except Exception as e:
...
if __name__ == "__main__":
  # =====================================================
    # REGISTER ONEWEST MAIL REMINDER BLUEPRINT
    # (Must be AFTER ow_mail_bp is defined above)
    # =====================================================
    try:
        app.register_blueprint(ow_mail_bp, url_prefix="/ow_mail")
        print("✅ ONEWEST Mail Reminder Blueprint registered at /ow_mail")
        
        # Setup 8:00 AM daily scheduler
        from apscheduler.schedulers.background import BackgroundScheduler
        ...
        scheduler.add_job(
            ...
            id='ow_daily_mail_8am',
            name='ONEWEST Daily Mail Auto-Send',
            replace_existing=True
        )
        scheduler.start()
        print("✅ ONEWEST Mail Scheduler: Daily auto-send at 8:00 AM IST")
        
    except Exception as e:
        print(f"⚠️  ONEWEST Mail module warning: {e}")
