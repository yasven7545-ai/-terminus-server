"""
sln_pm_daily_routes.py
══════════════════════════════════════════════════════════════════
SLN TERMINUS — Property Management Daily Log Routes
All routes prefixed with /sln/pm/daily
Blueprint: sln_pm_daily_bp
══════════════════════════════════════════════════════════════════
...
SENDER_PASSWORD = "xaottgrqtqnkouqn"          # Gmail App Password (16-char)
RECEIVER_EMAILS = [
...
#  BLUEPRINT + DATA DIR
# ══════════════════════════════════════════════════════════════
sln_pm_daily_bp = Blueprint("sln_pm_daily_bp", __name__)

BASE_DIR = Path(__file__).parent.resolve()
DATA_DIR = BASE_DIR / "static" / "data" / "pm_daily"
...
def _log_path(date_str: str) -> Path:
    return DATA_DIR / f"pm_log_{date_str}.json"


def _smtp_send(msg: MIMEMultipart) -> None:
    """
    Robust SMTP send using Gmail STARTTLS + SSL context.
    Raises specific smtplib exceptions so callers can surface proper errors.
    """
    ...
    with smtplib.SMTP(SMTP_SERVER, SMTP_PORT, timeout=20) as server:
        ...
        print(f"✅ Mail sent to {RECEIVER_EMAILS}")


def _handle_smtp_error(e: Exception):
    """Return a clear JSON error for any SMTP failure."""
    if isinstance(e, smtplib.SMTPAuthenticationError):
        return jsonify({"success": False,
                        "error": (
                            "Gmail authentication failed. "
                            "Check that: (1) 2-Step Verification is ON for "
                            "maintenance.slnterminus@gmail.com, "
                            "(2) the App Password 'xaottgrqtqnkouqn' is current, "
                            "(3) 'Less secure app access' is NOT blocking App Passwords."
                        )}), 500
...
#  PAGE ROUTE
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily")
@sln_login_required
...
#  API: SAVE
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/save", methods=["POST"])
@sln_login_required
def sln_pm_daily_save():
    try:
        ...
        log = {
            ...
            "property":   data.get("property", "SLN TERMINUS"),
        ...
        print(f"✅ SLN PM Daily saved: {date_str} by {log['author']}")
...
#  API: LOAD
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/load", methods=["GET"])
@sln_login_required
def sln_pm_daily_load():
...
#  API: HISTORY
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/history", methods=["GET"])
@sln_login_required
def sln_pm_daily_history():
    try:
        logs = []
        for f in sorted(DATA_DIR.glob("pm_log_*.json"), reverse=True):
            try:
                with open(f) as fh:
                    d = json.load(fh)
                logs.append({
...
#  API: EXPORT PDF
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/export_pdf", methods=["POST"])
@sln_login_required
def sln_pm_daily_export_pdf():
    try:
        data     = request.get_json(force=True) or {}
        date_str = data.get("date",     datetime.now().strftime("%Y-%m-%d"))
        shift    = data.get("shift",    "Full Day")
        author   = data.get("author",   "N/A")
        prop     = data.get("property", "SLN TERMINUS")
        tasks    = data.get("tasks",    [])
        remarks  = data.get("remarks",  "")

        if not REPORTLAB_OK:
            html = _build_print_html(date_str, shift, author, prop, tasks, remarks)
        ...
        doc.build(_build_pdf_story(date_str, shift, author, prop, tasks, remarks))
        buf.seek(0)
        return send_file(buf, mimetype="application/pdf",
                         as_attachment=True,
                         download_name=f"SLN_PM_Daily_{date_str}.pdf")
...
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/send_today", methods=["POST"])
@sln_login_required
def sln_pm_daily_send_today():
    """
    Send today's PM log immediately to all receivers.
    Accepts inline task data in the POST body (from the current unsaved form)
    OR loads from disk if no tasks are supplied.
    Body: { "date", "shift", "author", "property", "tasks", "remarks" }
    """
    try:
        data     = request.get_json(force=True) or {}
        date_str = data.get("date", datetime.now().strftime("%Y-%m-%d"))
        ...
        html_body = _build_today_mail_html(
            date_str, shift, author, prop, tasks, remarks, total, done, high
        )

        msg            = MIMEMultipart("alternative")
        msg["Subject"] = f"[SLN Terminus] PM Daily Log — {date_str}"
        msg["From"]    = f"SLN Terminus MMS <{SENDER_EMAIL}>"
...
#  API: MONTHLY MAIL
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/send_mail", methods=["POST"])
@sln_login_required
def sln_pm_daily_send_mail():
    """Send aggregated monthly summary to all receivers."""
    try:
        now       = datetime.now()
        month_str = now.strftime("%B %Y")
        prefix    = now.strftime("pm_log_%Y-%m-")
        logs      = []
        ...
        msg            = MIMEMultipart("alternative")
        msg["Subject"] = f"[SLN Terminus] PM Monthly Summary — {month_str}"
        msg["From"]    = f"SLN Terminus MMS <{SENDER_EMAIL}>"
        msg["To"]      = ", ".join(RECEIVER_EMAILS)
        msg.attach(MIMEText(html_body, "html"))
...
#  API: DELETE LOG
# ══════════════════════════════════════════════════════════════
@sln_pm_daily_bp.route("/sln/pm/daily/delete", methods=["POST"])
@sln_login_required
def sln_pm_daily_delete():
...
def _build_today_mail_html(date_str, shift, author, prop,
                            tasks, remarks, total, done, high):
    # Sort H → M → L
    ...
    task_rows = "".join(
        f"""<tr style="background:{'#ffffff' if i%2==1 else '#f8fafc'};border-bottom:1px solid #e5e7eb;">
    ...
    remarks_block = (
        f'<div style="margin:0 28px 24px;background:#eff6ff;'
        f'border-left:3px solid #00c2ff;padding:12px 16px;'
        ...
        f'letter-spacing:1px;text-transform:uppercase;color:#0369a1;">'
...
<div style="max-width:680px;margin:auto;background:#fff;border-radius:10px;
  ...
  <div style="background:#0b0e14;padding:24px 28px 20px;border-bottom:3px solid #00c2ff;">
    <div style="color:#00c2ff;font-size:10px;letter-spacing:2px;text-transform:uppercase;margin-bottom:6px;">{prop} · Property Management</div>
  ...
  <div style="padding:20px 28px 10px;">
    <div style="font-weight:800;font-size:11px;color:#0b0e14;margin-bottom:10px;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #00c2ff;padding-bottom:4px;">Task Log</div>
  </div>
  ...
  <div style="padding:14px 28px;background:#f6f8fc;border-top:1px solid #e8edf5;font-size:10px;color:#aaa;">
    SLN Terminus MMS — Auto-generated &nbsp;·&nbsp;
...
def _build_monthly_mail_html(month_str, logs, total_tasks, total_completed, total_high):
    rows = "".join(
        f"""<tr style="border-bottom:1px solid #e8edf5;">
          <td style="padding:8px 12px;font-size:12px;color:#0b0e14;
                     font-weight:600;font-family:monospace;">{l['date']}</td>
          ...
          <td style="padding:8px 12px;font-size:12px;font-weight:700;
                     color:#0b0e14;">{l.get('task_count',0)}</td>
...
<div style="max-width:640px;margin:auto;background:#fff;border-radius:10px;
  ...
  <div style="background:#0b0e14;padding:24px 28px 20px;">
    <div style="color:#00c2ff;font-size:10px;letter-spacing:2px;
                text-transform:uppercase;margin-bottom:6px;">SLN Terminus · Property Management</div>
  ...
  <table style="width:100%;border-collapse:collapse;">
    <thead><tr style="background:#f6f8fc;">
      <th style="padding:9px 12px;text-align:left;font-size:9px;letter-spacing:1px;
                 text-transform:uppercase;color:#607585;font-weight:700;
                 border-bottom:1px solid #e8edf5;">Date</th>
      <th style="padding:9px 12px;text-align:left;font-size:9px;letter-spacing:1px;
                 text-transform:uppercase;color:#607585;font-weight:700;
                 border-bottom:1px solid #e8edf5;">Shift</th>
      <th style="padding:9px 12px;text-align:left;font-size:9px;letter-spacing:1px;
                 text-transform:uppercase;color:#607585;font-weight:700;
                 border-bottom:1px solid #e8edf5;">Tasks</th>
      <th style="padding:9px 12px;text-align:left;font-size:9px;letter-spacing:1px;
                 text-transform:uppercase;color:#607585;font-weight:700;
                 border-bottom:1px solid #e8edf5;">Reported By</th>
    </tr></thead>
    <tbody>{rows}</tbody>
  </table>
...
#  PDF STORY BUILDER
# ══════════════════════════════════════════════════════════════
def _build_pdf_story(date_str, shift, author, prop, tasks, remarks):
    styles = getSampleStyleSheet()
    story  = []
    BRAND  = colors.HexColor("#00c2ff")
    ...
    story.append(Paragraph(
        f"{prop}  ·  Date: {date_str}  ·  Shift: {shift}  ·  By: {author}", sub_s))
    ...
    story.append(Paragraph(
        f"Generated by SLN Terminus MMS · "
...
def _build_print_html(date_str, shift, author, prop, tasks, remarks):
    rows = "".join(
...
<title>SLN PM Daily — {date_str}</title>
<style>
  body{{font-family:Arial,sans-serif;font-size:11px;margin:20mm;color:#1a2c3d;}}
  h1{{font-size:16px;margin-bottom:2px;}} .sub{{color:#607585;font-size:9px;margin-bottom:12px;}}
  hr{{border:none;border-top:2px solid #00c2ff;margin:8px 0 12px;}}
  table{{width:100%;border-collapse:collapse;}}
  th{{background:#0b0e14;color:#fff;padding:5px;font-size:9px;letter-spacing:1px;}}
  ...
  .remarks{{background:#f0f8ff;border:1px solid #00c2ff;padding:10px;
            margin-top:12px;white-space:pre-wrap;line-height:1.6;}}
  @media print{{@page{{size:A4;margin:15mm;}}}}
</style></head>
<body onload="window.print()">
<h1>PROPERTY MANAGEMENT DAILY REPORT</h1>
<div class="sub">{prop} · {date_str} · Shift: {shift} · By: {author}</div>
<hr/>
<table>
...
<p style="font-size:8px;color:#aaa;margin-top:20px;">
  SLN Terminus MMS · {datetime.now().strftime('%d %b %Y %H:%M')} · Confidential
</p>
</body></html>"""